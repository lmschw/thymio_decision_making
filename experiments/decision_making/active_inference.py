...
class ActiveInferenceBaselineExperiment:
    ...
    async def _tick(self):
            self.tick_count += 1
            # Wall-clock time, for cross-robot alignment: tick_count is a
            # local, unsynchronized per-robot counter (unlike ARGoS's single
            # shared simulation clock), so post-hoc analysis needs a real
